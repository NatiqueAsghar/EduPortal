...
function Table({ modelName }) {
  ...
  useEffect(() => {
    fetchData();
  }, [modelName]); // Added modelName as dependency
  ...
  const fetchData = async () => {
    try {
      ...
      const gridColumns = Object.keys(firstRow || {}).map((col) => ({
        ...
        width: 125,
        flex: col === "name" || col === "email" ? 1 : undefined, // Make some columns flexible
      }));
      ...
      // Add analytics button ONLY for class table
  ...
  // Listen for refresh-table event
  useEffect(() => {
